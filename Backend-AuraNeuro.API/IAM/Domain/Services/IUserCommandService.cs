...
namespace Backend_AuraNeuro.API.IAM.Domain.Services;
...
public interface IUserCommandService
{
    ...
    Task<(long id, string token)> Handle(SignInCommand command);
    ...
    Task<(long id, string token)> Handle(SignUpCommand command);
}